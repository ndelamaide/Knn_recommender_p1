...
object Baseline extends App {
  ...
  if (conf.train() contains "100"){
    ...
    println("")
    spark.close()
  }else { // Execution took too long for submission sorry!
        // Save answers as JSON
    def printToFile(content: String, 
                    location: String = "./answers.json") =
      Some(new java.io.PrintWriter(location)).foreach{
    ...
    conf.json.toOption match {
      ...
      case Some(jsonFile) => {
        var answers = ujson.Obj(
          ...
          "B.1" -> ujson.Obj(
            "1.GlobalAvg" -> ujson.Num(3.5264), // Datatype of answer: Double
            "2.User1Avg" -> ujson.Num(3.6330),  // Datatype of answer: Double
            "3.Item1Avg" -> ujson.Num(3.8882),   // Datatype of answer: Double
            "4.Item1AvgDev" -> ujson.Num(0.3027), // Datatype of answer: Double
            "5.PredUser1Item1" -> ujson.Num(4.0468) // Datatype of answer: Double
          ),
          "B.2" -> ujson.Obj(
            "1.GlobalAvgMAE" -> ujson.Num(0.8422), // Datatype of answer: Double
            "2.UserAvgMAE" -> ujson.Num(0.8421),  // Datatype of answer: Double
            "3.ItemAvgMAE" -> ujson.Num(0.7417),   // Datatype of answer: Double
            "4.BaselineMAE" -> ujson.Num(0.8421)   // Datatype of answer: Double
          ),
          "B.3" -> ujson.Obj(
            "1.GlobalAvg" -> ujson.Obj(
